...
#Initializing pygame
pygame.init()
pygame.display.set_caption("Nether Mirror")
#Defining the macros
...
##character classes definitions Start
...
##character classes definition End
##object classes definition Start
...
##Collisions and move functions Start
...
##Collisions and move Functions End

##functions that return list of objects
#Start
...
##functions to spawn objects and Zombies Start
def spawnZom(s):
    zom = [mZombie(4, 4, s),
           mZombie(9, 8, s),
           mZombie(16, 5, s),
...
##functions to spawn objects and Zombies End  
...
def main(window):
    ...
    pref=GUI(window)
    prefReflect=pref+"Reflect"

    zombies=spawnZom(block_size)
    player = Player(pref,block_size/2, HEIGHT//2-block_size, 50, 50)
 
    netherPlayer=NetherPlayer(prefReflect,player,50,50)
    ...
    while run:
        ...
        if ((player.rect.right - offset_x >= WIDTH - scroll_area_width) and player.x_vel > 0) and player.rect.right<58*block_size:
